...
const PaymentInfo: React.FC<Props> = ({ formData, handleChange, errors, handleBlur }) => {
	return (
		<>
				...
				<div className={fieldsRowClasses}>
					<div className={inputContainerClasses}>
						...
						{errors.currentAccount && (
							<p className="text-red-600 text-sm font-medium">{errors.currentAccount}</p>
						)}
					</div>
					<div className={inputContainerClasses}>
						<InputField
						...
						/>
						{errors.corAccount && (
							<p className="text-red-600 text-sm font-medium">{errors.corAccount}</p>
						)}
					</div>
				</div>
			</div>

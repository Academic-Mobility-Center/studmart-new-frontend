	...
	const validate = () => {
		...
		if (!profession) {
			...
			newErrors.profession =
				'Неверный формат профессии. Пример корректного значения: "Инженер", "Ведущий инженер", "Менеджер (по продажам)". Начинается с заглавной буквы, допускаются пробелы, дефисы и уточнения в скобках.';
		}
	...
	return (
		...
		>
			...
			<div className="flex flex-col gap-1">
				...
				/>
				{errors.university && (
					<p className="text-red-600 text-sm font-medium mt-1">{errors.university}</p>
				)}
			</div>
			<div className="flex flex-col gap-1">
				<InputField
				...
				/>
				{errors.profession && (
					<p className="text-red-600 text-sm font-medium mt-1">{errors.profession}</p>
				)}
			</div>
			<div className="flex flex-col gap-1">
				<SelectField

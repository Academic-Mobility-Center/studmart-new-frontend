...
const StudId: React.FC = () => {
	...
	const year =
		date.getFullYear() +
		(fetchStudent?.course?.yearsBeforeEnding && fetchStudent?.course?.yearsBeforeEnding > 0
			? fetchStudent?.course?.yearsBeforeEnding
			: 0);
	const userData = {
		...
		<>
			...
			>
				...
				<div className="flex justify-start items-start flex-row px-[30px] relative z-100">
					...
					>
						<label
							...
							className="border bg-[#f8f8f8] box-border flex justify-center items-center flex-col 
						...
						>
							...
							/>
							{isLoading && (
								<div className="animate-spin h-6 w-6 border-t-2 border-solid border-[#888888] rounded-full mt-2"></div>
							)}
						</label>
					...
					<div className="w-[171px] grow-0 shrink-0 basis-auto box-border ml-10 pb-[26px] gap-[15px] flex flex-col">
						<div className="flex-column">
							...
							<p className="text-[#032C28] text-xl font-bold">
						...
						<div className="flex-column">
							...
							<p className="text-[#032C28] text-xl font-bold">
								{userData?.course?.yearsBeforeEnding}
							</p>
						</div>
					</div>
				</div>
				<div className="relative">
					<Link
						href="profile"
						className="absolute inset-0 flex justify-between items-center px-4 z-10"
					>
						...
						<div className="box-border flex justify-start items-center flex-row gap-2.5">
							<Image
								...
								width={32}
								height={32}
								className="w-6 h-6 flex grow-0 shrink-0 basis-auto box-border"
							/>
							<p className="[font-family:Mulish,sans-serif] text-sm font-normal text-[#888888] grow-0 shrink-0 basis-auto m-0 p-0">
								Редактировать
							</p>
						</div>
					</Link>
